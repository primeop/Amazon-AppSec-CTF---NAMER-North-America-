...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (login($_POST['email'], $_POST['password'])) {
        ...
        $email = $_POST['email'];
        $error = "Wubba Lubba Dub Dub! Those credentials aren't from this dimension!";
    }
}
?>
<!DOCTYPE html>
<html>
<head>
    <title>Portal Access | Citadel of Ricks</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="form-container">
        <h2>Dimension C-137 Access</h2>
        <form method="POST" onsubmit="this.querySelector('button').classList.add('loading')">
            <input type="email" name="email" placeholder="Interdimensional Email" required value="<?= htmlspecialchars($email ?? '') ?>"
                   autocomplete="email">
        ...
        <a href="register.php" class="register-link">Register for Multiverse Access</a>
    </div>
    <script>
